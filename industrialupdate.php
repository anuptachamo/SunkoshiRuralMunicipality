...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_produces)){
        $produces_err = "Please enter the produce number.";     
    } elseif(!ctype_digit($input_contact)){
    ...
    if(empty($name_err) && empty($dropdown_err) && empty($no_of_staffs_err) && empty($establish_date_err) && empty($description_err) && empty($address_err) && empty($contact_err) && empty($produces_err)){
        ...
        $sql = "UPDATE industrial SET name=?, dropdown=?, no_of_staffs=?, establish_date=?, description=?, address=?, contact=?, produces=? WHERE id=?";
        ...
        if($stmt = $mysqli->prepare($sql)){
            ...
            $stmt->bind_param("ssssssssi", $param_name, $param_dropdown, $param_no_of_staffs, $param_establish_date, $param_description, $param_address, $param_contact, $param_produces, $param_id);
            // Set parameters
            $param_name = $name;
    ...
    if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
        ...
        $sql = "SELECT * FROM industrial WHERE id = ?";
        if($stmt = $mysqli->prepare($sql)){
            // Bind variables to the prepared statement as parameters
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group <?php echo (!empty($no_of_staffs_err)) ? 'has-error' : ''; ?>">
                            ...
                            <input type="number" name="no_of_staffs" class="form-control" value="<?php echo $no_of_staffs; ?>">
